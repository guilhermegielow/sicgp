 ...
 */
	function nice($date_string = null, $return = false) {
        ...
        setlocale(LC_TIME,'pt_BR','ptb');
        $ret = strftime('%A, %d de %B de %Y',$date);
	...
	function timeAgoInWords($datetime_string, $format = 'j/n/y', $backwards = false, $return = false) {
		...
		if ($months > 0) {
			...
			if ($weeks > 0) {
				...
				$relative_date .= $days > 0 ? ($relative_date ? ', ' : '') . $days . ' dia' . ($days > 1 ? 's' : '') : '';
			} elseif($days > 0) {
				// days and hours
				$relative_date .= ($relative_date ? ', ' : '') . $days . ' dia' . ($days > 1 ? 's' : '');
				$relative_date .= $hours > 0 ? ($relative_date ? ', ' : '') . $hours . ' hora' . ($hours > 1 ? 's' : '') : '';
			} elseif($hours > 0) {
				// hours and minutes
				$relative_date .= ($relative_date ? ', ' : '') . $hours . ' hora' . ($hours > 1 ? 's' : '');
				$relative_date .= $minutes > 0 ? ($relative_date ? ', ' : '') . $minutes . ' minuto' . ($minutes > 1 ? 's' : '') : '';
			} elseif($minutes > 0) {
				// minutes only
				$relative_date .= ($relative_date ? ', ' : '') . $minutes . ' minuto' . ($minutes > 1 ? 's' : '');
			} else {
				// seconds only
				$relative_date .= ($relative_date ? ', ' : '') . $seconds . ' segundo' . ($seconds != 1 ? 's' : '');
			}
		}
