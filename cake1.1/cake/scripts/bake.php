#!/usr/bin/php -q
<?php
/* SVN FILE: $Id: bake.php 4757 2007-04-04 08:24:30Z phpnut $ */
/**
 ...
 *
 * CakePHP(tm) :   Development Framework <http://www.cakephp.org/>
 ...
 */
	...
	function __bakeViews($controllerName, $controllerPath, $admin= null, $admin_url = null) {
		...
		$indexView .= "<h2>" . $pluralHumanName . "</h2>\n\n";
		...
		$indexView .= "\t\t<?php echo \$html->link('Detalhes','{$admin_url}/{$controllerPath}/view/' . \$".$singularName."['{$modelObj->name}']['{$modelObj->primaryKey}'])?>\n";
		$indexView .= "\t\t<?php echo \$html->link('Editar','{$admin_url}/{$controllerPath}/edit/' . \$".$singularName."['{$modelObj->name}']['{$modelObj->primaryKey}'])?>\n";
		$indexView .= "\t\t<?php echo \$html->link('Excluir','{$admin_url}/{$controllerPath}/delete/' . \$".$singularName."['{$modelObj->name}']['{$modelObj->primaryKey}'], null, 'Deseja realmente excluir : id ' . \$".$singularName."['{$modelObj->name}']['{$modelObj->primaryKey}'])?>\n";
		$indexView .= "\t</td>\n";
		$indexView .= "</tr>\n";
		$indexView .= "<?php endforeach; ?>\n";
		$indexView .= "</table>\n\n";
		$indexView .= "<ul class=\"actions\">\n";
		$indexView .= "\t<li><?php echo \$html->link('Inserir {$singularHumanName}', '{$admin_url}/{$controllerPath}/add'); ?></li>\n";
		$indexView .= "</ul>\n";
		$indexView .= "</div>";
		...
		$viewView .= "<h2>".$singularHumanName . "</h2>\n\n";
		$viewView .= "<dl>\n";
		$count = 0;
		foreach($fieldNames as $field => $value) {
		...
		$viewView .= "\t<li><?php echo \$html->link('Editar " . $singularHumanName . "',   '{$admin_url}/{$controllerPath}/edit/' . \$".$singularName."['{$modelObj->name}']['{$modelObj->primaryKey}']) ?> </li>\n";
		$viewView .= "\t<li><?php echo \$html->link('Excluir " . $singularHumanName . "', '{$admin_url}/{$controllerPath}/delete/' . \$".$singularName."['{$modelObj->name}']['{$modelObj->primaryKey}'], null, 'Deseja realmente excluir : id ' . \$".$singularName."['{$modelObj->name}']['{$modelObj->primaryKey}'] . '?') ?> </li>\n";
		$viewView .= "\t<li><?php echo \$html->link('Listar " . $pluralHumanName ."',   '{$admin_url}/{$controllerPath}/index') ?> </li>\n";
		$viewView .= "\t<li><?php echo \$html->link('Inserir " . $singularHumanName . "',	'{$admin_url}/{$controllerPath}/add') ?> </li>\n";
		foreach( $fieldNames as $field => $value ) {
			if( isset( $value['foreignKey'] ) ) {
				$otherModelName = $this->__modelName($value['model']);
				if($otherModelName != $currentModelName) {
					...
					$viewView .= "\t<li><?php echo \$html->link('Listar " . $otherSingularHumanName . "', '{$admin_url}/" . $otherControllerPath . "/index/')?> </li>\n";
					$viewView .= "\t<li><?php echo \$html->link('Inserir " . $otherPluralHumanName . "', '{$admin_url}/" . $otherControllerPath . "/add/')?> </li>\n";
				}
			}
		}
		$viewView .= "</ul>\n\n";
		...
		foreach ($modelObj->hasOne as $associationName => $relation) {
			...
			$viewView .= "<h3>". $otherPluralHumanName . ". relacionado(a)s</h3>\n";
			...
			$viewView .= "<?php endif; ?>\n";
			$viewView .= "<ul class=\"actions\">\n";
			$viewView .= "\t<li><?php echo \$html->link('Editar " . $otherSingularHumanName . "', '{$admin_url}/" .$otherControllerPath."/edit/' . \$".$singularName."['{$associationName}']['" . $modelObj->{$otherModelName}->primaryKey . "']);?></li>\n";
			$viewView .= "\t<li><?php echo \$html->link('Inserir " . $otherSingularHumanName . "', '{$admin_url}/" .$otherControllerPath."/add/');?> </li>\n";
			$viewView .= "</ul>\n";
			$viewView .= "</div>\n";
		}
		...
		foreach($relations as $associationName => $relation) {
			...
			$viewView .= "\t\t<?php echo \$html->link('Detalhes', '{$admin_url}/" . $otherControllerPath . "/view/' . \$".$otherSingularName."['{$otherModelObj->primaryKey}']);?>\n";
			$viewView .= "\t\t<?php echo \$html->link('Editar', '{$admin_url}/" . $otherControllerPath . "/edit/' . \$".$otherSingularName."['{$otherModelObj->primaryKey}']);?>\n";
			$viewView .= "\t\t<?php echo \$html->link('Excluir', '{$admin_url}/" . $otherControllerPath . "/delete/' . \$".$otherSingularName."['{$otherModelObj->primaryKey}'], null, 'Deseja realmente excluir : id ' . \$".$otherSingularName."['{$otherModelObj->primaryKey}'] . '?');?>\n";
			$viewView .= "\t</td>\n";
			$viewView .= "</tr>\n";
			$viewView .= "<?php endforeach; ?>\n";
			$viewView .= "</table>\n";
		...
		//-------------------------[ADD]-------------------------//
		$addView = null;
		$addView .= "<h2>Novo " . $singularHumanName . "</h2>\n";
		...
		$addView .= $this->generateSubmitDiv('Adicionar');
		$addView .= "</form>\n";
		$addView .= "<ul class=\"actions\">\n";
		$addView .= "<li><?php echo \$html->link('Listar {$pluralHumanName}', '{$admin_url}/{$controllerPath}/index')?></li>\n";
		foreach ($modelObj->belongsTo as $associationName => $relation) {
			...
			if($otherModelName != $currentModelName) {
				...
				$addView .= "<li><?php echo \$html->link('Detalhes " . $otherPluralName . "', '{$admin_url}/" .$otherControllerPath."/index/');?></li>\n";
				$addView .= "<li><?php echo \$html->link('Adicionar " . $otherPluralName . "', '{$admin_url}/" .$otherControllerPath."/add/');?></li>\n";
			}
		}
		$addView .= "</ul>\n";
		//-------------------------[EDIT]-------------------------//
		$editView = null;
		$editView .= "<h2>Editar " . $singularHumanName . "</h2>\n";
		...
		$editView .= $this->generateSubmitDiv('Salvar');
		$editView .= "</form>\n";
		$editView .= "<ul class=\"actions\">\n";
		$editView .= "<li><?php echo \$html->link('Excluir','{$admin_url}/{$controllerPath}/delete/' . \$html->tagValue('{$modelObj->name}/{$modelObj->primaryKey}'), null, 'Deseja realmente excluir : id ' . \$html->tagValue('{$modelObj->name}/{$modelObj->primaryKey}'));?>\n";
		$editView .= "<li><?php echo \$html->link('Listar {$pluralHumanName}', '{$admin_url}/{$controllerPath}/index')?></li>\n";
		foreach ($modelObj->belongsTo as $associationName => $relation) {
			$otherModelName = $this->__modelName($associationName);
			if($otherModelName != $currentModelName) {
				$otherControllerName = $this->__controllerName($otherModelName);
				...
				$otherSingularName = $this->__singularName($associationName);
				$otherPluralName = $this->__pluralHumanName($associationName);
				$editView .= "<li><?php echo \$html->link('Detalhes " . $otherPluralName . "', '{$admin_url}/" .$otherControllerPath."/index/');?></li>\n";
				$editView .= "<li><?php echo \$html->link('Adicionar " . $otherPluralName . "', '{$admin_url}/" .$otherControllerPath."/add/');?></li>\n";
			}
		}
		$editView .= "</ul>\n";
 ...
 */
	...
	function __bakeActions($controllerName, $admin = null, $admin_url = null, $wannaUseSession = 'y') {
		...
		$actions .= "\t\t\t\$this->Session->setFlash('ID inválido para {$singularHumanName}.');\n";
		$actions .= "\t\t\t\$this->redirect('{$admin_url}/{$controllerPath}/index');\n";
		} else {
		$actions .= "\t\t\t\$this->flash('ID inválido para {$singularHumanName}', '{$admin_url}/{$controllerPath}/index');\n";
		}
		$actions .= "\t\t}\n";
		...
		if (low($wannaUseSession) == 'y' || low($wannaUseSession) == 'yes') {
		$actions .= "\t\t\t\t\$this->Session->setFlash('".$this->__singularHumanName($currentModelName)." inserido com sucesso.');\n";
		$actions .= "\t\t\t\t\$this->redirect('{$admin_url}/{$controllerPath}/index');\n";
		} else {
		$actions .= "\t\t\t\t\$this->flash('{$currentModelName} salvo.', '{$admin_url}/{$controllerPath}/index');\n";
		}
		$actions .= "\t\t\t} else {\n";
		if (low($wannaUseSession) == 'y' || low($wannaUseSession) == 'yes') {
		$actions .= "\t\t\t\t\$this->Session->setFlash('Por favor, corrija os erros abaixo.');\n";
		}
		...
		$actions .= "\t\t\tif(\$this->{$currentModelName}->save(\$this->data)) {\n";
		if (low($wannaUseSession) == 'y' || low($wannaUseSession) == 'yes') {
		$actions .= "\t\t\t\t\$this->Session->setFlash('".Inflector::humanize($currentModelName)." editado com sucesso.');\n";
		$actions .= "\t\t\t\t\$this->redirect('{$admin_url}/{$controllerPath}/index');\n";
		} else {
		$actions .= "\t\t\t\t\$this->flash('{$currentModelName} salvo.', '{$admin_url}/{$controllerPath}/index');\n";
		}
		...
		if (low($wannaUseSession) == 'y' || low($wannaUseSession) == 'yes') {
			$actions .= "\t\t\t\$this->Session->setFlash('".$this->__singularHumanName($currentModelName)." Excluído: id '.\$id.'');\n";
			$actions .= "\t\t\t\$this->redirect('{$admin_url}/{$controllerPath}/index');\n";
		} else {
			$actions .= "\t\t\t\$this->flash('{$currentModelName} excluído: id '.\$id.'.', '{$admin_url}/{$controllerPath}/index');\n";
		}
		$actions .= "\t\t}\n";
		$actions .= "\t}\n";
		$actions .= "\n";
 ...
 */
	function generateAreaDiv($tagName, $prompt, $required=false, $errorMsg=null, $cols=60, $rows=10,  $htmlOptions=null ) {
		...
		$str .= "\t<?php echo \$html->tagErrorMsg('{$tagName}', 'Por favor informe {$prompt}.');?>\n";
 ...
 */
	function generateCheckboxDiv($tagName, $prompt, $required=false, $errorMsg=null, $htmlOptions=null ) {
		...
		$str = "\t<?php echo \$html->tagErrorMsg('{$tagName}', 'Por favor marque {$prompt}.');?>\n";
		$divClass = "optional";
 ...
 */
	function generateDate($tagName, $prompt, $required=false, $errorMsg=null, $size=20, $htmlOptions=null, $selected=null ) {
		$str = "\t<?php echo \$html->dateTimeOptionTag('{$tagName}', 'DMY' , 'NONE', \$html->tagValue('{$tagName}'), " . $this->__attributesToArray($htmlOptions) . ");?>\n";
		$str .= "\t<?php echo \$html->tagErrorMsg('{$tagName}', 'Por favor selecione {$prompt}.');?>\n";
		$strLabel = "\n\t<?php echo \$form->labelTag('{$tagName}', '{$prompt}');?>\n";
		$divClass = "optional";
 ...
 */
	function generateDateTime($tagName, $prompt, $required=false, $errorMsg=null, $size=20, $htmlOptions=null, $selected = null ) {
		$str = "\t<?php echo \$html->dateTimeOptionTag('{$tagName}', 'DMY' , '12', \$html->tagValue('{$tagName}'), " . $this->__attributesToArray($htmlOptions) . ");?>\n";
		$str .= "\t<?php echo \$html->tagErrorMsg('{$tagName}', 'Por favor selecione {$prompt}.');?>\n";
		$strLabel = "\n\t<?php echo \$form->labelTag('{$tagName}', '{$prompt}');?>\n";
		$divClass = "optional";
